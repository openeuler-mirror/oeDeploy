...
- `[plugin]`如果为插件下载地址，则先下载到缓存路径，再初始化到指定路径
- `[plugin]`如果为插件名称，优先在本地源查找，后在远端源查找，下载后初始化到指定路径（开发中）

| 选项               | 简写 | 是否必需 | 功能说明                                                     |
| ------------------ | ---- | -------- | ------------------------------------------------------------ |
| `--project [path]` | `-p` | Y        | 项目路径，若不存在则创建                                     |
...
| `/etc/oedp/config/repo/cache/`（开发中）    | 插件源索引文件缓存路径 |
| `/etc/oedp/config/repo/repo.conf`（开发中） | 插件源配置文件         |
